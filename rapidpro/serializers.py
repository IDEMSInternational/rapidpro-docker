...
class RunBulkActionSerializer(WriteSerializer):
    DELETE = "delete"
    ACTIONS = (DELETE,)

    runs = fields.RunField(many=True)
    action = serializers.ChoiceField(required=True, choices=ACTIONS)

    def validate_runs(self, value):
        if not value:
            raise serializers.ValidationError("Runs can't be empty.")
        return value

    def save(self):
        runs = self.validated_data["runs"]
        action = self.validated_data["action"]
