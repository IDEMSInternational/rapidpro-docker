...
class ExplorerView(OrgPermsMixin, SmartTemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["endpoints"] = [
            ...
            RunActionsEndpoint.get_write_explorer(),
...
class RunActionsEndpoint(BulkWriteAPIMixin, BaseEndpoint):
    """
    ## Bulk Run Updating

    A **POST** can be used to perform an action on a set of runs in bulk.

    * **runs** - the run UUIDs (array of up to 100 strings).
    * **action** - the action to perform, a string one of:

        * `delete` - permanently delete the runs.

    Example:

        POST /api/v2/run_actions.json
        {
            "runs": ["55b6606d-9e89-45d1-a3e2-dc11f19f78df", "bef96b71-865d-480a-a660-33db466a210a"],
            "action": "delete"
        }
    ...
    serializer_class = RunBulkActionSerializer

    @classmethod
    def get_write_explorer(cls):
        return {
            "method": "POST",
            "title": "Update Multiple Runs",
            "url": reverse("api.v2.run_actions"),
            "slug": "run-actions",
            "fields": [
                {"name": "runs", "required": True, "help": "The UUIDs of the runs to update"},
                {"name": "action", "required": True, "help": "One of the following strings: delete"},
            ],
        }
